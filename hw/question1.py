...
def swedish(): #    Jacob look here for #3
    #x, y, angle vel repeated 9 times
    data = [
        [0, 5, 0],
        [.3, 0, 0],
        [0, -5, 0],
    ...
    dfEverything = pd.DataFrame(data, columns=['X', 'Y', 'Angle'])
    dfX = pd.DataFrame(dataX, columns=['X'])
    dfY = pd.DataFrame(dataY, columns=['Y'])
    dfAngle = pd.DataFrame(dataAngle, columns=['Angel'])
